...
class Service(Reader):
    ...
    def __init__(self):
        ...
        self.crime_rate_colums = ['살인검거율', '강도검거율', '강간검거율', '절도검거율', '폭력검거율']
        self.crime_colums = ['살인', '강도', '강간', '절도', '폭력']
    ...
    def save_police_norm(self):
        ...
        f.fname = '/police_pos'
        polis_pos = r.csv(f)
        police = pd.pivot_table(polis_pos, index='구별', aggfunc=np.sum)
        ...
        for i in self.crime_rate_colums:
        ...
        x = police[self.crime_rate_colums].values
        ...
        """
                ...
                """
        police_norm = pd.DataFrame(x_scaled, columns=self.crime_colums, index=police.index)
        police_norm[self.crime_rate_colums] = police[self.crime_rate_colums]
        police_norm['범죄'] = np.sum(police_norm[self.crime_rate_colums], axis=1)
        police_norm['검거'] = np.sum(police_norm[self.crime_colums], axis=1)
        police_norm.to_csv('./saved_data/police_norm.csv', sep=',', encoding='UTF-8')

    def save_folium_map(self):

        # police_norm.csv, kr-states.json, crime_in_seoul.csv, police_pos.csv
        f = self.f
        r = self.r
        ...
        for i in police_pos.index:
            folium.Circle([police_pos['lat'][i], police_pos['lng'][i]],
